...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif update.callback_query:  # Если это вызов из callback (например, "Назад")
        await update.callback_query.edit_message_text(text, reply_markup=reply_markup)
        logger.info(f"Главное меню отображено для пользователя {update.callback_query.from_user.id}")
    else:
